...
const input = 'workflows/n8n/prd-genie-realistic-multi-source-canary-v0.3.json';
const output = 'workflows/n8n/prd-genie-realistic-multi-source-canary-v0.4.json';
const childId = process.argv[2];
if (!childId) throw new Error('Child v1.4.1 workflow ID is required');

const workflow = JSON.parse(fs.readFileSync(input, 'utf8'));
workflow.name = 'PRD Genie - Realistic Multi-Source Requirement Extraction Canary v0.4';
workflow.versionId = crypto.randomUUID();
const child = workflow.nodes.find((node) => node.type === 'n8n-nodes-base.executeWorkflow');
const oldName = child.name;
child.name = 'Execute Requirement Extractor Child v1.4.1';
child.parameters.workflowId.value = childId;
child.parameters.workflowId.cachedResultName = 'PRD Genie - Requirement Extractor Child v1.4.1';

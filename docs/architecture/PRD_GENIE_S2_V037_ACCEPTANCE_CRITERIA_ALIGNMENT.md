...
Langfuse trace `078fcc42f9011251801340d8f7ddb2c2` identified a systematic Stage 5/6 display-contract defect rather than evaluator timing noise. The approved namespace is `FR-003` refresh, `FR-004` role-based access, `FR-005` PDF, `FR-006` Excel, and `FR-007` responsive access; only `NFR-001` and `NFR-002` exist. The held artifact instead reused shifted legacy display IDs, added category/status filtering beyond the approved `FR-002` preset/custom date-range scope, allowed acceptance-criterion wording to inflate PDF priority, and rendered several non-grammatical story capabilities while claiming full synchronization.

A no-run local correction now exists as Stage 5 `v0.2.3 - Approved Namespace Alignment Candidate` and Stage 6 `v0.2.7 - Evaluator Alignment Candidate`. Stage 5 renders its ten-section PRD dynamically from approved elements instead of overriding them with the stale canonical body. Stage 6 uses the corrected FAC namespace, derives priority only from the functional-requirement statement, and normalizes all 11 capability phrases. The new evaluator-alignment suite and all existing v0.3.7 contract suites pass. Nothing was deployed and no further model run was launched at this checkpoint.

The evaluator-alignment candidates were then imported as isolated callable workflows `nyAUEOZ9lgxtVMAM` (Stage 5) and `dLDH4ChV0qAogbRD` (Stage 6). The isolated parent was saved as `v0.3.7 final evaluator-alignment canary` with only those two references changed. One final controlled execution, parent `11544` / run `RUN-S2-11545-16e7090e`, completed operationally in 2m 34.597s. Stages 5 and 6 and the one-shot score query passed. Faithfulness improved from `0.82` to `0.94`, above the `0.90` threshold, while hallucination improved from `0.62` to `0.22` but remained above the `0.10` maximum. The Agreement Gate correctly held release; Stage 7, Drive delivery, and sizing were not invoked, and no retry was launched.

The remaining root cause is extractor identifier instability across otherwise equivalent runs. The earlier diagnostic run assigned refresh/access/PDF/Excel/responsive as `FR-003`/`FR-004`/`FR-005`/`FR-006`/`FR-007`; run `RUN-S2-11545-16e7090e` assigned the same concepts as `FR-006`/`FR-003`/`FR-004`/`FR-007`/`NFR-003`. Therefore any packet-level hard-coded display-ID translation can become false on the next run. The next correction must use the current approved element `item_id` selected by semantic bucket for every FAC and story display source, with no fixed ID map. No additional canary is authorized until that deterministic dynamic-ID correction passes locally.

The dynamic-ID correction was implemented locally as Stage 5 `v0.2.4 - Dynamic Display IDs Candidate`. It selects current-run functional, non-functional, and acceptance-criterion items by semantic feature pattern, derives criterion and feature display IDs from those matched items, preserves every approved filtering source, and contains no fixed `prd_source_ids` map. All v0.3.7 local suites passed. The candidate was imported and published as isolated workflow `WvENlg3lF5iOyCFO`, and only the isolated parent Stage 5 reference was changed.

The single confirmation canary, parent execution `11551` / run `RUN-S2-11552-16e7090e`, passed the unchanged intake, Gap Analyzer, and human approval chain, then failed immediately at the parent Stage 5 invocation with `Cannot convert undefined or null to object`. The new child recorded no execution, so the failure occurred while n8n resolved or invoked the newly published sub-workflow rather than inside the Stage 5 generation code. No PRD, Story, evaluator, Stage 7, Drive, or sizing work ran, and no retry was launched. This is an invocation/runtime blocker; the prior artifact-producing run remains `RUN-S2-11545-16e7090e`.

The binding subsequently resolved without a workflow-definition change. Parent step execution `11556` invoked Stage 5 successfully, proving that the earlier failure was transient n8n binding/version availability rather than a Stage 5 code defect. The first full follow-up, parent execution `11562` / run `RUN-S2-11563-16e7090e`, then exposed a separate deterministic extractor-variation edge: the approved responsive-launch decision was emitted as approved constraint `CON-004` rather than an NFR, while the dynamic feature selector accepted only functional, NFR, and acceptance-criterion types. Stage 5 failed closed before evaluation or downstream delivery.

Stage 5 was corrected to accept an approved `constraint` as a valid dynamic feature source. The exact failed Stage 5 input passed as retry execution `11568` in 3.154s. The final closure canary, parent execution `11569` / run `RUN-S2-11570-16e7090e`, succeeded end to end in 2m 41.878s. Stage 5 generated the corrected ten-section PRD with 7 feature groups and 11 `FAC-*` criteria; Stage 6 generated the 3 Epic / 7 Feature / 11 User Story hierarchy with 11 linked `SBAC-*` criteria; the one-shot score set was complete; faithfulness was `1.00`; hallucination was `0.02`; and the Agreement Gate authorized release. Stage 7, seven-file Drive delivery, and non-blocking 11/11 sizing all passed. Production v0.3.6 remains unchanged pending explicit v0.3.7 promotion approval.

PR #14 preserved the complete candidate and evidence chain and merged to `main` as `c2fcdf2b58846de12cda9acdd495a9d4fa8fc75c`. Following explicit promotion approval, the already validated manual parent `LpRK4ibx4rXZ1IEs` was renamed `S2_ Dynamic Realistic Six-Source Main Orchestrator v0.3.7 - Production` and adopted as the production entrypoint. The v0.3.6 parent `MmnN5zOlFv2Cwwcl` was left unchanged as the rollback checkpoint.

The first post-promotion smoke attempt, parent execution `11578`, stopped before Stage 5 because the existing `Google Drive account 13` OAuth refresh had expired. After the owner reconnected that unchanged credential, the single controlled retry succeeded as parent execution `11580` / run `RUN-S2-11581-16e7090e` in 2m 35.984s. Faithfulness was `1.00`, hallucination was `0.01`, the Agreement Gate authorized release, Stage 7 passed, seven Drive artifacts were delivered, and non-blocking sizing passed 11/11. This is the accepted v0.3.7 production smoke checkpoint.

...
Fresh calibrated execution `10300` passed T1-T10 at 10/10 with 100% groundedness, zero unsupported claims, and 10/10 Code Evaluator passes. The LLM judges now evaluate the dedicated `substantive-v1` projection instead of the complete administrative envelope. Average LLM faithfulness is `0.940` and average LLM hallucination is `0.008`; 9/10 cases meet both provisional per-trace thresholds. Keep the agreement gate in shadow mode pending human adjudication of T3, where the judge disputes whether the source establishes a contradiction between five-second auto-refresh and minimizing API calls.

# Requirement Extractor Prompt v1.3
...
### Complete-with-optional-clarification rule added in v1.3

Return `complete` when every explicitly stated persona, requirement, constraint, dependency, deadline, risk, and relationship has been faithfully captured and there is no contradiction or missing source content that prevents extraction. A stated requirement may remain broad and still be fully extracted. Optional clarification that would improve future testability does not by itself make the extraction `partial`. For example, `End users need a simplified view` is a stated functional requirement: preserve it exactly, link it to the End users persona, and do not return `partial` solely because `simplified` could later be defined more precisely. Use `partial` only when the source fragment cannot reliably support a requirement item, a material ambiguity prevents faithful classification, or an unresolved contradiction remains.
